...
class MainController(
    ...
    @GetMapping("/editExternalPart/{partListId}/{listId}")
    fun getExternalPart(@PathVariable("partListId") partListId: String,
                        @PathVariable("listId") listId: String,
                 model: Model): String{
        model.addAttribute("partList",
                questionnaireService.getPartListQuestionnaireByPartListId(partListId))
        model.addAttribute("listId",listId)
        return "editExternalPartName"
    }

    @PostMapping("/editExternalPart/{listId}")
    fun editExternalPart(@PathVariable("listId") listId: String,
                     partList: PartListQuestionnaire): String{
        questionnaireService.saveEditPartList(partList)
